...
> 版本/日期以 **2026 年 5 月** 為準：OpenCode 採 MIT 授權,**15 萬以上 GitHub star**（確切數字各來源不一、易過時),首次公開發布於 2025 年 6 月,目前釋出 **v1.15.x**（v1.15.12,2026-05-28),節奏為每週多次發版。TUI 在正式環境仍為 **Go + Bubble Tea**;遷移至團隊自研 **OpenTUI**（Zig 核心 + TypeScript 綁定）的計畫見 issue #2956,屬 *規劃/PoC* 階段、尚未出貨——因此已稱其為「Ink/TypeScript」的評論為時過早,而稱整體為「Go CLI」者則錯標了 server/CLI（後者為 TS on Bun,僅 TUI binary 是 Go）。GitHub 於 2026 年 1 月透過 Copilot 訂閱加入官方 OpenCode 支援。比較對象 **Gemini CLI** 已於 I/O（2025-05-19）退役,由 **Antigravity CLI** 接替（Gemini CLI 約於 2026-06-18 停止服務）。下方成本數字為 model API 估計,並非 OpenCode 授權費（並無此費用）。
...
| **語言 / runtime** | server/CLI 為 TS on Bun;TUI 目前為 **Go + Bubble Tea**（OpenTUI/Zig 遷移規劃中） | TS/Node | Python | TS/Node |
...
- **Server。** 一個持久行程（`opencode serve`),以 TypeScript 寫成、跑在 **Bun** runtime 上,提供 HTTP API（預設埠 **4096**),以 **Server-Sent Events** 串流即時事件,並以 **SQLite（Drizzle ORM）** 持久化 session。它擁有 agent loop、工具執行與 session 狀態,並暴露如 `/session`、`/provider`、`/mcp`、`/lsp`、`/event`（SSE）、`/file` 等路由。由於狀態存於 server 端,**session 可在終端機斷線、SSH 中斷、筆電休眠後續存**——你重新連線即可接續。這是單一行程代理無法比擬的結構性特性。
...
- **TUI 實作——持續變動的目標。** OpenCode 的 TUI 是一個 **Go + Bubble Tea** binary（其與 charm 相近的源頭;見 §2),由 TypeScript CLI 在啟動 server 後 spawn。截至 2026 年 5 月,Go TUI 仍是正式環境的預設。遷移至團隊自研 **OpenTUI**（以 **Zig** 寫成、附 TypeScript 綁定的原生終端 UI 核心）的規劃見 issue #2956,但仍屬 *規劃/PoC*,過渡期保留 Go-TUI 後援。因此已描述為「Ink/TypeScript TUI」的評論為時過早,而稱整體為「Go CLI」者則錯標了 server/CLI（後者為 TS on Bun）。
- **Model 層。** 供應商存取透過 **Models.dev** 目錄 + **Vercel AI SDK** 抽象,因此新增供應商是設定、而非寫程式,model 以 `providerID/modelID` 參照。這讓「75+ 供應商」名副其實而非行銷詞——同一個 agent loop 可對 Claude、GPT、Gemini、OpenRouter 路由的 model、Bedrock、Copilot 訂閱 model、廠商自家的 **OpenCode Zen** pay-as-you-go 服務,或本地 Ollama 端點執行。
...
憑證存於本地 `~/.local/share/opencode/auth.json`。你可用任意供應商的原始 API key 驗證,或對支援的供應商以瀏覽器 **OAuth** 驗證（TUI 內 `/connect`）。訂閱可作為後端沿用——**GitHub Copilot** 訂閱自 2026 年 1 月起有官方支援,**Anthropic Claude Pro/Max** 登入亦有提供,但沿用訂閱 token 仰賴社群 OAuth 外掛、且處於 ToS 灰色地帶（Anthropic 將訂閱 token 限定於官方 client）——依賴前請先查核。
...
- **發版變動。** 每週多次發版（2026 年 5 月一週內 v1.15.7→v1.15.12),據報功能在版本間會出問題,而進行中的 Go-TUI → OpenTUI 遷移更添不穩風險。文件、部落格與肌肉記憶失準;第三方指南常描述較舊或為時過早的架構。預期它是這四個工具中變動最快的表面。
- **資源使用。** GitHub issue 中有 TUI 占用 1 GB 以上 RAM、CPU 尖峰與偶發 session 卡住的回報。另須注意:部分最嚴厲的批評（失控的 token 計費、用於產生標題的遙測）針對的是第三方「Oh My Opencode」設定層,*而非* 核心 OpenCode——勿混為一談。
